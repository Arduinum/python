...
class Cell:
    ...
    def __str__(self):
        if self.cell < 0:
            return 'Ошибка - отрицательных клеток быть не может!'
        else:
            return str(int(self.cell))
    # сложение
    ...
    def __truediv__(self, other):
        return Cell(self.cell / other.cell)
    # групперовка ячеек по рядам
    def make_order(self, rows):
        self.rows = rows
        self.num_int = self.cell // self.rows
        self.modulo = self.cell % self.rows
        self.result = ((self.rows * '*') + '\n') * self.num_int
        if self.modulo >= 1:
            self.modulo *= '*'
            self.result += self.modulo
        return self.result
